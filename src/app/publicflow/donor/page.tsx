...
import { connectWallet, sendDonation, sendUSDCDonation, getTransparencyEvents, getTreasuryBalance, getAccountBalance, NGO_CONFIG } from "@/lib/web3";
...
export default function DonorPage() {
    ...
    const fetchData = async () => {
        try {
            ...
            if (!isSimMode) {
                ...
                setTreasuryBalance("42.5");
                setDonorBalance("0.05");
            }
        } catch (err) {
    ...
    const handleConnect = async () => {
        ...
        try {
            ...
            // Strict Donor Identity Check
            if (connectedAddress.trim().toLowerCase() !== NGO_CONFIG.donor.toLowerCase()) {
                alert(`Wrong Account Detected!\n\nPlease switch to Donor Wallet (Account 1: ${NGO_CONFIG.donor.substring(0, 10)}...) to proceed.`);
                return;
            }
    ...
    const handleDonate = async () => {
        ...
        try {
            if (donationMethod === 'usd') {
                const tx = await sendUSDCDonation(amount, NGO_CONFIG.treasury);
                setTrackingStep(2);
            } else {
                const tx = await sendDonation(amount, NGO_CONFIG.treasury);
                setTrackingStep(2);
            }
            setDonating(false);
            fetchData();
            ...
            <main className="min-h-screen bg-mesh flex flex-col items-center justify-center p-8 text-white">
                ...
                >
                    ...
                    <p className="text-white/40 text-sm mb-10 leading-relaxed font-medium">
                        Connect your <b>Donor Wallet (Account 1)</b> to support the {NGO_CONFIG.name} mission with transparent tracking.
                    </p>
                                ...
                                <div className="w-full max-w-3xl">
                                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-16 relative">
                                        <span className="text-[10px] font-black uppercase tracking-[0.3em] text-emerald-500 block mb-2">
                                            {trackingStep === 1 && "Verifying Transfer to NGO..."}
                                            {trackingStep === 2 && "Vaulted in NGO Treasury"}
                                            {trackingStep === 3 && "Impact Achieved: Funds Disbursed"}
                                        </span>
                                        <h3 className="text-2xl font-bold">
                                            {trackingStep === 1 && "Security Check"}
                                            {trackingStep === 2 && "Confirmed Receipt"}
                                            {trackingStep === 3 && "Mission Success"}
                                        </h3>
                                    </motion.div>
                                    ...
                                    <div className="flex justify-between items-center relative py-10">
                                        ...
                                        <div className="flex flex-col items-center gap-4 relative">
                                            <div className={`w-20 h-20 rounded-[32px] glass flex items-center justify-center transition-all duration-500 ${trackingStep >= 2 ? 'border-emerald-500/50 bg-emerald-500/5 shadow-[0_0_50px_rgba(16,185,129,0.1)]' : 'border-white/10'}`}>
                                                <Shield className={`w-8 h-8 ${trackingStep >= 2 ? 'text-white' : 'text-white/10'}`} />
                                            </div>
                                        ...
                                        <div className="flex-1 relative h-2">
                                            ...
                                            {trackingStep > 2 && <div className="w-full h-0.5 bg-blue-500/30 absolute top-0" />}
                                        </div>
                                        <div className="flex flex-col items-center gap-4 relative">
                                            <div className={`w-16 h-16 rounded-3xl glass flex items-center justify-center transition-all duration-500 ${trackingStep >= 3 ? 'border-blue-500 shadow-[0_0_40px_rgba(59,130,246,0.2)]' : 'border-white/10'}`}>
                                                <Globe className={`w-6 h-6 ${trackingStep >= 3 ? 'text-blue-500' : 'text-white/20'}`} />
                                            </div>
                                            <span className="text-[9px] font-bold text-white/30 uppercase tracking-widest">Mission Field</span>
                                        </div>
                                    </div>
                                </div>
                            ) : (
                ...
                <div className="lg:col-span-4 flex flex-col gap-8">
                    <section className="glass p-8 rounded-[40px] border-white/5 relative overflow-hidden group">
                        ...
                        <div className="space-y-8">
                            ...
                            <button onClick={handleDonate} disabled={donating} className={`w-full py-6 rounded-3xl font-black uppercase tracking-widest text-[11px] transition-all flex items-center justify-center gap-3 ${trackingStep > 0 ? 'bg-white/10 text-white/40 border border-white/10' : 'bg-emerald-500 text-black shadow-lg hover:scale-[1.02]'}`}>
                                {donating ? <Activity className="w-5 h-5 animate-spin" /> : <ArrowUpRight className="w-5 h-5" />}
                                <span>{trackingStep > 0 ? "Tracking..." : "Confirm & Send"}</span>
                            </button>
                        </div>
                    </section>

                    <section className="glass p-8 rounded-[40px] flex-1">
                        <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-white/30 mb-8">Audit Ledger</h3>
                        <div className="space-y-4 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                            {displayedEvents.map((event) => (
                                <div key={event.id} className="p-4 rounded-3xl bg-white/[0.02] border border-white/5 flex justify-between items-center">
                                    <div>
                                        <div className="text-[11px] font-bold text-white/90">{event.name}</div>
